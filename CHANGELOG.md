...
- **可观测性**：新增救援时间线、服务健康状态、模块清单基线与最近变更识别；展示层只读取状态，不修改模块。
- **快照**：支持命名、固定、差异查看与选择性恢复；固定快照不参与自动裁剪。
- **安全模式**：新增“仅下一次启动”安全模式。先原子写入精确事务清单，再写本次涉及的 `disable` 标记；成功启动或用户取消时仅恢复该清单中的标记，不触碰白名单、第三方或原先已禁用模块。
- **诊断**：新增默认脱敏的诊断 ZIP；优先使用系统 `zip`，并兼容 Magisk、KernelSU、APatch 常见 BusyBox 路径。
...
- **UI**：WebUI 改为 Miuix 系统设置式浅色分组和手机端五项底部导航；中文 / EN 在窄屏始终可见。
- **完整性**：模块 `versionCode` 升级时自动建立当前版本基线，不再把正常更新误报为 `REVIEW_REQUIRED`；同版本文件变化继续报告 `COMPROMISED`。
- **守护**：修复完整性守护脚本权限和启动兼容；升级时仅迁移完整性派生状态，不影响配置、快照和启动记录。
- **展示**：完整性最近检查改为可读时间。

# v3.4.0（正式版）

- **移除**：彻底移除高风险脚本扫描、自动拦截、隔离、风险告警和脚本目录锁定；RescueX 聚焦于模块状态救援，避免 TypeScript/JavaScript 正常代码误报。
- **安全**：恢复清单缺失、为空或格式异常时，拒绝恢复；不会恢复全部禁用模块。
- **安全**：自动 APP 解冻已禁用；不会删除 `package-restrictions.xml`。
- **一致性**：跨进程救砖锁与 `rescue.plan`、`rescue.commit`、`rescue.rollback` 事务记录。
- **补丁**：patch flag 使用 TTL；无可信快照不自动回滚；失败不清失败计数。
- **看门狗**：多信号健康观察；自动路径仅一次普通重启，不进入 Recovery、不使用 SysRq。
- **完整性**：基线版本不匹配时人工复核，不自动重建。
- **默认**：首次安装 `DRY_RUN=true`，快照保留数统一为 12。

# v3.3.0-r7 (2026-07-22)

### Fixed

- **TA_utl 误报修复**：修正模块自身隐藏目录路径的匹配顺序，`/data/adb/modules/.<module_id>` 不再被识别为删除其他模块根目录。
- **自清理脚本回归保护**：新增可见自身目录、隐藏自身目录、混合删除其他模块目录的检测测试。
- **清除提醒交互修复**：后端返回 `ALERT_CLEARED=1` 或 `ALERT_ALREADY_CLEAR=1`，与 WebUI 的成功判定保持一致。

## v3.3.0-r6 (2026-07-22)

### Fixed

- **当前启动周期禁用修复**：脚本拦截命中模块后，除了写入下一次启动使用的 `disable` 标记，还会停止当前已经运行的该模块脚本进程。
- **拦截时序修复**：将脚本拦截放在正常启动判定后的最早位置，避免模块脚本继续运行造成“日志显示已禁用、当前进程仍在执行”。

## v3.3.0-r5 (2026-07-22)

### Fixed

- **脚本拦截禁用闭环修复**：高风险脚本拦截现在使用统一模块禁用函数，并验证 `disable` 标记实际写入成功。
- **隐藏副本同步修复**：针对运行时复制到 `.<module_id>` 的模块，同步禁用隐藏副本并锁定其启动脚本，避免拦截后仍继续执行。
- **WebUI 操作统一**：单模块手动禁用与救砖、脚本拦截共用同一套禁用逻辑。

## v3.3.0-r4 (2026-07-22)

### Fixed

- **模块自清理误报修复**：允许模块升级或卸载流程清理自身目录及隐藏目录，同时继续拦截删除其他模块根目录、系统目录或共享存储的脚本。
- **TS/TrickyStore 兼容性修复**：修复 `TA_utl` 的 `service.sh`、`uninstall.sh` 和更新脚本被高风险 Shell 检测器错误禁用的问题。
