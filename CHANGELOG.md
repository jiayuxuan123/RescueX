# v3.4.0-r1-beta（安全灰度版）

- **安全**：移除“恢复证据缺失时恢复所有禁用模块”的兜底；恢复改为 fail-closed。
- **安全**：自动 APP 解冻已禁用；不再删除 `package-restrictions.xml` 或其备份。
- **一致性**：新增跨进程救砖锁及 `rescue.plan`、`rescue.commit`、`rescue.rollback` 事务记录。
- **补丁**：patch flag 改为 TTL 状态；无可验证补丁快照时不自动回滚；回滚失败不清失败计数。
- **看门狗**：采用多信号健康观察；自动路径仅普通重启一次，移除自动 Recovery/SysRq。
- **完整性**：版本与既有基线不匹配时改为人工复核，禁止自动重建基线。
