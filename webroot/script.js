/* RescueX v3.5.10 - WebUI controller
...
const APP_VERSION_CODE = 35020;
// v3.5.10: update queues are owned by the Root manager; token changes are
...
const I18N = {
    zh: {
        ...
        update_notice_desc: 'v3.5.10 修复模块更新期间的错误重启与误救砖：RescueX 不再接管 Root 管理器更新队列；BOOT_TOKEN 仅用于诊断，不再单独触发自动救砖。',
        check_update: '检查更新',
        ...
        view_releases: '版本发布',
        about_desc: 'RescueX 通过监控启动失败次数和开机超时，自动禁用问题模块以救砖。兼容 Magisk / KernelSU / APatch。v3.5.10：管理器更新队列完全隔离，启动 token 仅用于诊断与事务归属。',
        loading: '加载中...',
        // 状态文本
        status_ok: '系统正常',
    ...
    en: {
        ...
        update_notice_desc: 'v3.5.10 fixes erroneous reboot and rescue during module updates: RescueX no longer takes over Root-manager update queues, and BOOT_TOKEN is diagnostic only rather than a standalone rescue trigger.',
        ...
        about_desc: 'RescueX monitors boot failures and auto-disables problematic modules to break bootloops. Compatible with Magisk / KernelSU / APatch. v3.5.10 isolates manager update queues and uses boot tokens only for diagnostics and transaction ownership.',
        loading: 'Loading...',
  ...
  [ -f "$d/common.sh" ] && { echo "$d"; exit 0; }
    ...
    setDefaultSubtitle() {
        ...
        el.classList.remove('easter-note');
            el.textContent = this.lang === 'zh' ? '自动救砖守护 v3.5.10' : 'Automatic Boot Rescue v3.5.10';
    }
...
manual_generate_rescue_decision_report`, EXEC_REPORT_TIMEOUT_MS);
    ...
    showOnboarding(contentHash, ackPath) {
        ...
        title.textContent = isZh ? 'RescueX v3.5.10 更新须知' : 'RescueX v3.5.10 Update Notice';
        ...
        const sections = isZh ? [
            ...
            { title: 'v3.5.10 Safety Fixes', items: [
        ...
        btn.onclick = async () => {
            ...
            overlay.remove();
            this.toast(isZh ? '欢迎使用 RescueX v3.5.10' : 'Welcome to RescueX v3.5.10', 'success');
        };
